...
<script>
    import {getParentByClassName,getChildrenByClassName,getSiblingsByClassName} from '../js/Node.js'
    export default {
        props:[
            'options',
            'status',  // 是否有子集
        ...
        methods:{
            toggleUplistContent(e){
                let parent = getParentByClassName(e.target,'yh-uplist-set'),
                    siblings = getSiblingsByClassName(parent,'yh-uplist-set'),
                    title = getParentByClassName(e.target,'yh-uplist-set-title'),
                    content = getSiblingsByClassName(title,'yh-uplist-set-content')[0],
                    icon = getChildrenByClassName(title,'icon')[0],
                    i = 0,
                    stitle = null,
                    scontent = null,
                    sicon = null,
                ...
                for(i = 0; i < siblings.length; i++){
                    stitle = getChildrenByClassName(siblings[i],'yh-uplist-set-title')[0]
                    scontent = getChildrenByClassName(siblings[i],'yh-uplist-set-content')[0]
                    sicon = getChildrenByClassName(stitle,'icon')[0]
